...
# 동기식 chatGPT 함수
def chatgpt_sync(system_prompt, user_prompt):
...
# 비동기식 chatGPT 함수
async def chatgpt_async(system_prompt, user_prompt):
    ...
    try:
        ...
        async def generate(): # 제너레이터 생성 
            async for chunk in response:
                content = chunk.choices[0].delta.content or ""
                yield content  # 클라이언트에게 각 스트림 조각을 전송
...
def topic_classification(user_input):
    system_prompt = """
        ...
        """
    return chatgpt_sync(system_prompt, user_input )
...
def extract_arrv_dest(user_input): #user input 에서 출발지와 도착지 출력  
    system_prompt = """
            ...
            """
    return chatgpt_sync(system_prompt =system_prompt, user_prompt= user_input )

async def handle_weather_topic(user_input):
    ...
    system_prompt = (f"You are a helpful assistant, and you will kindly answer questions about current weather. "
              ...
              "이 날씨 정보를 다 출력할 필요는 없고, 주어진 질문인 '{usr_input}'에 필요한 답만 해줘 ")
    result = await chatgpt_async(system_prompt, user_input)
...
def handle_trans_topic(user_input):
    ...
    if dict_string:
        ...
        print("from_to_dict", from_to_dict)
        print(get_route_description(from_to_dict, TMAP_API_KEY, KAKAO_MAP_API_KEY))
    ...
    print("handle_trans_topic - result", result)
    return Response(stream_message(result), content_type='text/plain')


# 비동기식으로 ELSE 주제 처리
async def handle_else_topic(client, usr_input):
    prompt = ("You are a helpful assistant."
              "사용자들은 한국어로 질문할 거고, 너도 한국어로 대답해야돼")
    result = await chatgpt_async(client, usr_input)
...
async def llm():
    ...
    print("topic:", topic)
    if topic == "WEATHER":
        return await handle_weather_topic(user_input)
    elif topic == "TRANS":
        return handle_trans_topic(user_input)
    elif topic == "ELSE":
        return await handle_else_topic(user_input)
    else:
...
def make_title(): # 대화의 타이틀 생성
    ...
    title = chatgpt_sync(system_prompt, user_input)
