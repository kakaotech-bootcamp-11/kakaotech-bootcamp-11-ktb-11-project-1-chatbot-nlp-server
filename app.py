...
def topic_classification(user_input):
    system_prompt = """
        ...
        """
    return cls_chatgpt(system_prompt, user_input )
...
def extract_arrv_dest(user_input): #user input 에서 출발지와 도착지 출력  
    system_prompt = """
            ...
            """
    return cls_chatgpt(system_prompt =system_prompt, user_prompt= user_input )
...
def make_title(): # 대화의 타이틀 생성
    ...
    title = stream_chatgpt(system_prompt, user_input)
...
if __name__ == '__main__':
    ...
    retriever = pdf_retriever(pdf_path, MODEL_VERSION)
    """try:
